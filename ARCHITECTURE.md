...
| Layer | Modules | Notes |
|---|---|---|
| **App assembly** | `main.py` (~640 lines) | FastAPI app, middleware, capability-gate exception handler, startup (queue worker + pollers), home page, `/live` `/power` `/carbon`, `/ui-config.js`, `/queue`(+`-status`), cooldown-decision. Nothing feature-shaped lives here. |
