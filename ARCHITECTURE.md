...
| Layer | Modules | Notes |
|---|---|---|
| **App assembly** | `main.py` (~640 lines) | FastAPI app, middleware, capability-gate exception handler, startup (queue worker + pollers), home page, `/live` `/power` `/carbon`, `/ui-config.js`, `/queue`(+`-status`), `/lab-session/*` (toggle + reservations), cooldown-decision. Nothing feature-shaped lives here. |
| **Feature routes** (18) | `routes_video.py`, `routes_llm.py`, `routes_rag.py`, `routes_image.py`, `routes_enhance.py`, `routes_benchmark.py`, `routes_budget.py`, `routes_findings.py`, `routes_results.py`, `routes_settings.py`, `routes_demo.py`, `routes_methodology.py`, `routes_auth.py`, `routes_decode.py` (Lab rig console + public batch pages), `routes_rem.py` (`/prepare-rem`, share links), `routes_audience.py` (hidden `/audience`), `routes_privacy.py`, `routes_mockups.py` (TEMP landing mockups, CR-059) | One flat module per feature, each an `APIRouter`: its routes + page template + `run_*_job` orchestration. **Never import `main`.** `main.py` keeps a commented alias block (benchmark.py + tests reach `run_job`, `run_llm_compare_models_job`, … through it). `routes_budget.py` = `/video/budget` calculator + `/video/budget/reconfigure` (Lab re-cal). |
...
| **Access spine** | `audience.py`, `capabilities.py`, `queue_control.py`, `lab_reservations.py` | `audience.tier(request)` → Anonymous/Member/Lab. `capabilities._REQUIRED_TIER` **is** the security policy; routes declare capabilities via `Depends(requires(CAP))`, never tiers. `queue_control` is the single enqueue path + worker (+ the lab-session flag). `lab_reservations` (CR-083) = the courtesy calendar in `data/lab_reservations.json` whose 15 s ticker raises/lowers that flag for due slots. |
| **Auth** | `auth.py`, `email_send.py` (+ `routes_auth.py`) | CR-001 magic-link sign-in (allowlist `data/members.json`), Gmail SMTP. |
| **Measurement** | `video.py`, `llm.py`, `image_gen.py`, `rag.py`, `pixop.py`, `benchmark.py`, `parity.py`, `rem_prep.py`, `precalibration.py`, `quality.py` | One per workload. Byte-stable through the whole refactor (energy-imperceptibility rule). `parity.py` = the re-runnable encode-parity / energy-quality calibration harness (codec × CPU/GPU × bitrate × clip, reusing `video.run_single`-style measurement + terminal VMAF; writes a fingerprinted artifact under `results/calibration/`). |
| **Decode rig** | `rig.py` (device registry `RIG`, plug/boot state machine, claim-screen, idle auto-off, C2 wake), `decode_run.py` (templates incl. loop / iso-bitrate / `net_*` families → per-device bench configs → `decode_bench/bench.py` subprocesses, envelope assembly), `decode_batch.py` (campaign collation), `lg.py` (webOS SSAP + WoL), `origin_control.py` (owns the `:8123` Range-correct clip origin as a service child), `idle_wait.py` (the shared settle loop used by CR-070 and the rig guard) | The rig side of OWL (S58 →). Harness proper lives in `decode_bench/` (`bench.py` drivers adb/ssh/webos, `origin.py`, `campaign.py`). |
...
| **Analysis** | `confidence.py`, `energy.py` (the one ΔE = ΔW×Δt implementation), `carbon.py`, `canonical.py`, `curated.py`, `analytics.py` (GDPR-safe visit counts) | Shared CI/traffic-light model; CO₂e context (reference-only). |
| **Storage** | `persist.py`, `settings.py`, `sources.py`, `model_catalog.py`, `corpus_manifest.py`, `findings.py`, `reproduce.py`, `uploads.py` (3-way upload retention shared by /video, /enhance-run, /decode, /prepare-rem) | Flat-file JSON under `results/`. `persist._SUMMARISERS` is the mode→summariser dispatch (see result envelope). `settings.json` is **live state** — never include it in feature commits. LLM `MODELS` dicts are live views — never edit as literals. |
...
run_*_job (routes_<feature>.py)          ← orchestration: stages, cooldowns, jobs[] updates
  └ measurement module (video.py, …)     ← focus mode (video/image/pixop/parity; NOT llm/rag) → pre-job idle guard (CR-070) → baseline (`baseline_polls`×1s) → task → polls
...
change; rollback anchor: tag `v1.0.0` (2026-07; `main` is far ahead (190+ commits as of 2026-09) — the pre-refactor `v0.8.7` tag is historical).
