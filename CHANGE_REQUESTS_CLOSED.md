...
## CR-018 · Historical CO₂e comparison — Tier 1 curated dates (Tiers 2/3 merged into CR-007)

**Status:** ✅ closed 2026-06-11. Tier 1 shipped 2026-05-03, commit `bf462c3` (S18 part 14). Tiers 2/3 were never started; their scope merged into active **CR-007** (the carbon variance study needs exactly the same cached historical dataset — one infrastructure, two outputs).

**Triggered by:** the live CO₂e card showed "now" only; the French grid's 2020→2024 lifecycle-intensity drop (Eco2mix series) is one of OWL's best stories and deserved a per-result "what would this run have emitted then" comparison.

### What Tier 1 delivered

Five curated historical dates (FR 2020-01 / 2020-06 / 2022-06 / 2024-01 / 2024-06) hard-coded in `carbon.HISTORICAL_INTENSITY`, generated by the one-shot `bin/fetch-historical-mix` helper, computed with the SAME `compute_intensity_from_mix` lifecycle method as the live path, and surfaced as the "Through history" block in the carbon comparison strip on every result card; documented on `/methodology`. A small well-chosen set of dates was judged a sharper story than a free date picker.

### What moved to CR-007

Tier 2 (cached month-resolution Eco2mix history + `carbon.historical_intensity(zone, year, month)` + `/carbon/historical` endpoint + year-month picker) and Tier 3 (interactive timeline scrubber) — see CR-007's Stage 1 in CHANGE_REQUESTS.md.

---

## CR-064 · /enhance-run revamp — upload, format/resolution controls, generated presets

**Status:** ✅ closed 2026-06-11 (S45). All 12 agreed-direction items shipped S40–S45; closing commit `088c249` (Pixop named on the page — the final CR-064 commit). Key commits: `5bded39` (core revamp: upload + SDR/HDR × SD/HD/4K generated-preset matrix + past runs), `4acf992` (4:2:0 default + advanced args editor), `ab6206a` (queue cancel/empty controls + /settings nav pass), `6a4d387`/`6f0b669`/`9668c51` (input normalization per Jon's prescription + normalize-idle stage + normalized preview proxy), `4195643` (compare normalizes via NUT pipe), `9460f20` (early normalization verdict: upload + picker badge), `8fdc8ec` (Jon's blessed pix_fmt list), `088c249` (member-contributed-technology naming). Tests 505 → 648 across the arc.

**Triggered by:** June 10 call with Jon (Pixop) + Tania — replace raw `.args` preset files and fixed staged clips with a user-facing flow: upload or pick a clip, choose output format (SDR/HDR) + Super Resolution target (SD/HD/4K), bitrate auto-coupled, preset selected silently.

### What shipped (the 12 items, compressed)

(1) Six static combos work on any input — no probe drives the UI; probe results are provenance only. (2) Preset generation from two Jon-authored colour templates × 3 targets × coupled CBR into `presets/generated/` (`preset_origin: generated`). (3) Output-format radio + SR dropdown, bitrate read-only, full args in `<details>`. (4) Member upload 1 GB / ≤60 s (ffprobe-enforced), Lab uncapped, keep-checkbox retention encoded in the filename, TTL sweep. (5) ffmpeg comparison side matched to the preset's actual output format (`_NVENC_PIX_FMT`). (6) NR-quality "subject to further refinement" footnote. (7) enhance/enhance_compare as first-class persisted results (summarisers, list/CSV/delete endpoints, Previous-runs section). (8) SI/TI complexity table default-collapsed. (9) 4:2:0 (HEVC Main10) output default — browser-decodable. (10) Advanced args editor → per-job `presets/custom/` file, `preset_origin: custom`; ffmpeg comparison derives from actual args. (11) Workload-aware Lab queue controls (enhance = `docker kill` per-job container; benchmark = cooperative flag; others honestly refused 409) + queue pause toggle (`36aa8ae`). (12) /settings anchor nav + collapsed bulky sections.

### Open questions — answered during the arc

**UGC tolerance (Jon's 2026-06-11 prescription):** ONE mechanism fixes both real-world failures (VFR × `forcecfr` mp4-muxer abort; `yuvj422p` decoder refusal) — conditional probe-driven lossless pre-normalization (yuv444p10le + `fps` CFR, FFV1/NUT, 2-pass-verified) running BEFORE the baseline window so its CPU cost never enters an energy figure; `_VERIFIED_PIX_FMTS` = Jon's blessed classic-formats list; Live 1× refuses normalizing inputs (honesty), compare paces them via a NUT pipe. Verified on the real failing clips. **hdr_4k abort:** root cause = VRAM ceiling (repro PASSED at 96.8% peak, 15,774/16,303 MiB) — Jon's OOM theory supported; exception-handling bug acknowledged by Pixop. **HDR→SDR tone-map:** explicitly deferred to Pixop's own dynamic tone-mapper. **Naming:** Pixop agreed to be named — member-contributed-technology framing, serve-time `{PARTNER_*}` tokens, independence statement + recruitment line.

### Residuals (promote to an active CR only if one becomes real work)

1. **Jon/Tania review session** → page sheds the "UNDER DEVELOPMENT" banner (the banner in the code is the standing reminder).
2. **`hdr_4k` — ✅ SHIPPED 2026-06-16 (un-excluded + throttled).** `_COMBO_EXCLUSIONS` is now empty; the combo generates and runs with Jon's two memory-throttle env vars (`_COMBO_ENV`, applied in `build_docker_cmd` for hdr_4k ONLY — every other combo runs bare, keeping its energy comparable to history). Result stamps `pixop_env`; the page shows a "mild throttle applied (hardware memory limit) — energy unaffected" note on the combo; `/methodology` carries the reduced-capacity note. Guarded by `test_combo_env_throttle` + `test_build_docker_cmd_injects_hdr4k_throttle`. Decision basis below (the measured A/B). Original repro log for Jon: `/srv/data/owl/pixop/logs/repro_hdr4k.log`. **NB OWL still runs image `2026.06.10`; if Jon's 2026-06-15 FFmpeg-8.1 build later lowers the default-env ceiling, the throttle could be dropped (remove the `hdr_4k` entry from `_COMBO_ENV`).** Restart pending to serve the page/methodology changes.
   - **Jon's env-var answer (2026-06-15 email):** two knobs throttle the pipeline to cut peak VRAM — `PIXOP_ASYNC_CAPACITY_SCALE=0.5` (input-buffer scale, default `1.5` → 3× smaller read-ahead) and `PIXOP_SR_PROCESSING_THREADS=2` (concurrent SR dispatcher threads, default `5`; the direct VRAM lever — each thread holds its own activation tensors). Jon warns throughput drops. **Energy caveat:** lower parallelism/lookahead changes the GPU power×time profile, so a run under these vars is **NOT energy-comparable** to a default-tuned pass — if `hdr_4k` is ever re-enabled this way it needs its own as-measured label in the result envelope (cf. frozen AMD baselines). Jon also migrated to FFmpeg 8.1 + new build but advises **keeping the audio pre-remux workaround** (`fa6cfda`) as more reliable.
   - **Measured A/B (2026-06-15 eve, `/tmp/owl_envvar_test.py` — no OWL edits, OWL's own baseline/poll/confidence harness, image `pixop/live:2026.06.10`, input `bbb_hd_clean.mp4` 1080p SDR 45s → 4K ×2 SR, all arms 🟢 n>100):**
     | combo | env | peak VRAM | fps | wall | ΔW | ΔE |
     |---|---|---|---|---|---|---|
     | SDR→4K | default | 9436 MiB / 57.9% | 24.87 | 112.7s | 395.1 | 12.37 Wh |
     | SDR→4K | throttled | 8308 MiB / 51.0% | 24.90 | 112.5s | 396.5 | 12.39 Wh |
     | HDR→4K | default | **15322 MiB / 94.0%** | 22.57 | 123.6s | 393.5 | 13.51 Wh |
     | HDR→4K | throttled | **13840 MiB / 84.9%** | 22.60 | 123.5s | 390.1 | 13.38 Wh |
     **Verdict:** (a) HDR→4K *completed rc=0 even at default* this run but at 94% VRAM (≈ the 96.8% repro — one allocation from rc 134); the env vars cut peak by ~1.45 GB (94.0→84.9%, ~2.5 GB headroom) — a legitimate path to making it reliable. (b) **Energy + throughput cost ≈ nil** — fps/wall/ΔW/ΔE all within run-to-run noise (≤1%); Jon's "performance will likely decrease" warning did NOT materialise on single-stream 4K SR (compute-bound, not buffer/thread-bound — his penalty likely shows on multi-stream/high-throughput). So the earlier "not energy-comparable" caveat is empirically negligible *for this workload* (the as-measured label still applies if shipped). VRAM sampled ~2.5 Hz → 94.0% is a floor (a sub-400 ms spike could be missed).
   - **×4.5 follow-up (2026-06-16, `/tmp/owl_sd4k_test.py`): `meridian_sd_dirty` 854×480 SDR → HDR 4K, batch AND Live 1×-paced, default vs throttled — all 4 rc=0.** Default peaked **89.9%** (14656 MiB), throttled **79.7%** (12990 MiB); fps 17.27 identical, ΔE ~16 Wh within noise. **Correction to the earlier caveat:** the ×4.5 (480p→4K) case uses *less* VRAM than ×2 (1080p→4K) — 89.9% vs 94.0% — because peak VRAM tracks **output resolution + model footprint, not input size / scale ratio** (a 480p source feeds smaller intermediate buffers). So 1080p→4K is the VRAM-worst of these, not the high-ratio SD case. **Live == batch** for VRAM (the `-re` pacer blocks on the pipe, bounding the input buffer; backpressure-OOM hypothesis disproved). **Could NOT reproduce a reported `meridian_sd_dirty → HDR-4K` hard failure** across batch+live (input needs no normalization — CFR yuv420p, no audio — so the test matched the page path exactly). The 2026-06-10 rc-134 abort was on an earlier image and/or the 1080p input at the edge; on `pixop/live:2026.06.10` HDR→4K now borderline-PASSES (6/6 runs today), suggesting the exclusion may be over-cautious — but the ~90–94% peaks are still one unlucky allocation from rc 134, which is exactly why the throttle (→ ~80–85%) is the right enabler. **NB OWL runs image `2026.06.10`; Jon's 2026-06-15 FFmpeg-8.1 build is NOT pulled.** Next if re-enabling: test the 1080p `meridian_hd_dirty`/`bbb_hd_dirty` → HDR-4K (the 94% case) under the throttle, and decide whether to pull Jon's newest image first.
...
**Status:** ✅ closed 2026-08-27 (S69/S70 overnight). Result: 🟢 **Repeatable** finding (SMPTE C19) — H.264≈HEVC 4.08 W device-total mean, AV1/VP9 5.27 W (+29 %), n=3 × 4 codecs × 3 content families (36 rows, 1 excluded), confirming the pilot's n=2/one-content estimate. Full record in JOURNAL.md S69 (2026-08-26 evening → 08-27 morning) and `~/dev/smpte-4951/digests/2026-08-appletv-vlc.md`. Spun off two follow-on artifacts that outlive this CR: the **screen map** (`rig_hdmi_inputs`, shipped same evening) and **CR-077** (device-onboarding idle-settle characterization tool, triggered by this campaign's settle-time incident). Residual open items live in the SMPTE digest, not here: a `rig.py` power/boot entry using pyatv `turn_on`/`turn_off` (currently manual); a native-AirPlay driver using upstream PR #2899 (receiver fetches the media, just needs the session held open — no VP9/VLC caveat if built); 4K/HDR untested. **Follow-up same day (2026-08-27, ~9h later):** the post-update elevated-baseline
question is resolved, not open — a rested, cold-booted re-check (park in tvOS Settings, full 4-min
settle, 5 min/1 s window, n=290) read mean 2.57 W, statistically the same as the tvOS 18 floor
(2.5–2.7 W); the overnight campaign's ~3.2–3.5 W plateau was recurring post-boot housekeeping
re-triggering on the ~4-min park/relaunch cycle, not a permanent property of tvOS 26.6. Never affected
the closed finding (device-total playing W).

**First attempt 2026-07-30:** pyatv Companion + AirPlay paired
