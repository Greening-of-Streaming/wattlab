...
#   - TESTING.md — pytest suite (1024 tests) + manual checklist · WATTLAB_SPEC.md — historical design intent
...
- S64 (08-17 pm): CR-073 campaigns = batches — reuse REM `batch_id`; `decode_batch.matrix` + `/decode/batch/{id}` (+json/csv, Anonymous) + campaign toggle + `persist.list_batch` + `envelope_version: 1` + decode single-store + `bin/stamp-decode-batch`; last night → `/decode/batch/aae11481804e`; DB stays CR-031 §1 (pain = presentation, not querying) · self-service: /decode/batches list, Lab Add-to-campaign + labels (persist.stamp_batch/list_batches); VP9 cross-type page NOT built (still in discussion). →1024.
