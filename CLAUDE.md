...
# See also: CHANGE_REQUESTS.md — active CRs only (18 entries). Tracks: A storage/persistence (CR-012 + CR-031 storage section + downstream CR-003 / CR-007), B confidence model (CR-028 Phase 2 — Tania reply pending), C widget extensions (CR-024, CR-035, deferred resume-job), D result rendering (CR-034 absorbed CR-013, plus CR-032 + CR-027), E small polish (CR-005), F strategic / exploratory (CR-008, CR-009, CR-018 T2/T3, CR-025, CR-029 + CR-033). See "Groupings & dependencies" appendix at end of CHANGE_REQUESTS.md for the full dependency map.
...
**Active CRs:** see `CHANGE_REQUESTS.md` (18 entries — CR-003 iso-energy, CR-004 graphing, CR-005 fan control, CR-007 carbon variance, CR-008 REM↔OWL, CR-009 web client, CR-012 calibration history, CR-018 T2/T3 historical, CR-024 probe button, CR-025 RT Linux, CR-027 tier copy, CR-028 confidence model, CR-029 encoding rigor, CR-031 portability, CR-032 per-mode CO₂e rows, CR-033 curated demo video, CR-034 unified results card *(absorbed CR-013 — prev-row click-to-expand is its Phase B)*, CR-035 encode progress bar). Closed CRs in `CHANGE_REQUESTS_CLOSED.md`. CR-019 resume-job hook deferred to a follow-up CR — capture if/when it surfaces in real visitor traffic. See "Groupings & dependencies" appendix at end of `CHANGE_REQUESTS.md` for cross-CR dependencies.
