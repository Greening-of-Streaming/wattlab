...
# Last updated: 2026-06-03 (Session 39 — CR-062 follow-up: fixed three /image compare-4-models bugs + audited the cooldown surface for single-sourcing. (1) "Cooldown shown as fixed 10s" despite wait-for-idle ON — the inline compare-page progress labels (image STAGE_LABELS + /llm CPU-vs-GPU strip) hardcoded {COOLDOWN_S}, which bakes to "10" regardless of the toggle; CR-062 only fixed the shared WL_*_STAGES. Fix = single toggle-aware {COOLDOWN_PAREN} token in _bake_durations, repointed both labels at it. (2) Only 3 of 4 models shown = NOT a render bug: sdxl-lightning (UNet-only, no model_index.json, first in panel) fails to load → runner records it in model_errors + continues, and its failure skipped the first cooldown (hence 2 cooldowns not 3); wlRenderImageCard now surfaces model_errors like wlRenderComparePanel does. (3) Progress strip misaligned — duplicate "cooldown" keys collapsed under indexOf → unique cooldown_<i> keys + pollJob maps the generic stage via current_model_idx. Audit verdict: cooldown BEHAVIOUR (power.cooldown_between_runs) + WORDING (_bake_durations tokens) + footer (wlCooldownSummary) are single-sourced; per-caller orchestration (reference_w, when-to-cool, result stamp-key) is not, and the result key is inconsistent (cooldowns list vs cooldown dict). +4 cooldown-label guard tests → 432 passing. Service restart needed for the label fix. NB: JOURNAL has no S38 full entry — CR-062 shipped with only its CLAUDE.md one-liner.)
