...
# Last updated: 2026-05-03 (Session 18)
...
- **S18 (2026-05-03):** **CR-010 closure** — carbon comparison strip gains a pinned home-zone reference row (Ember 2024 annual mean alongside the live headline) plus a one-line divergence note when today's grid is ≥25% cleaner/dirtier than the annual mean. Live-source explainer + mix-subheading dispatch on `HOME_ZONE` rather than hard-coding France/Eco2mix. **CR-006 closure** — AI workloads re-framed as "Beta · exploratory": landing-page nav re-labelled with explainer + per-button BETA tags, single-source `_BETA_CHIP` constant on /llm /rag /image h1s, Demo Tour gains an "Entering beta · exploratory" framing band before steps 2–4 (steps 2/3/4 h1s pick up the chip via {BETA_CHIP} placeholder substitution). **CR-011 partial — OWL-side artifacts** shipped: `bin/stage-on` (drain-with-timeout + flag + restart), `bin/stage-off` (wait-for-up + flag-down), `wattlab_service/static/maintenance.html`, `STAGING.md` workflow doc. System-side config (nginx vhost `/tmp/owl-maintenance` watch + `www-data` in `gos` group) **not yet applied — pending sudo run**. Variance recalibrated (variance_pct 1.08 → 3.62) with three new `*_bitrate_kbps` settings.json fields. `.gitignore` expanded for runtime data + sibling-project working copy; four `__pycache__/*.pyc` slips untracked.
