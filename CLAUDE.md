...
# Last updated: 2026-05-13 (Session 24 — GoS1 4TB data disk + 5th case fan; OWL data → /srv/data; CR-005 resolved; thermal-recovery probe; GPU-sensor chip-resolution fix; ElectricityMaps trial cross-check)
...
- Idle power: ~56-58W steady (S24 thermal-recovery probe, post-NVMe + 5th case fan; ~3-5W up on the old ~51-54W), brief drift to ~60-63W after sustained load. `variance_pct` 0.66–2.84% historically (calibration-dependent); the next variance recalibration at this HW baseline will pin `w_base` + `variance_pct`.
...
- [ ] **Variance recalibration at the post-S24 HW baseline** (NVMe + 5th case fan added ~3-5 W to idle). S24 thermal-recovery probe re-run (same 12-distance config as S21): full idle recovery by d≈5 s, steady idle ~56-58 W, within-window CV ~2.0% — so `variance_cooldown_s` of ~10 s is plenty (S22's 90 / the 40 default were overkill). Recommended run: n=24, `variance_cooldown_s=10`. *Prior runs for reference:* 2026-05-05 overnight n=24/cooldown=90/gpu_encode_max_s=90 → idle 2.41% / cpu 1.33% / gpu 4.77%; 2026-05-07 post-CR-022 quick n=6 (full encodes, no `-t` cap) → idle 2.26% / cpu 0.66% / gpu 0.95%.
