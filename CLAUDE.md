...
# Last updated: 2026-05-07 (Session 23 — CR-022 fully resolved + CR-026 anon-integrity)
...
# See also: CHANGE_REQUESTS.md — active CRs only. Active list: CR-003…CR-005, CR-007…CR-009, CR-012/013, CR-018 (T2/T3), CR-024, CR-025, CR-027…CR-029, CR-031, CR-032. CR-025 = exploratory on real-time Linux migration. CR-028 Phase 2 awaits Tania's reply (units-shape email sent 2026-05-07). CR-029 (encoding rigor) is highest active priority by tag.
# See also: CHANGE_REQUESTS_CLOSED.md — shipped CRs archive (CR-001, CR-001b, CR-002, CR-006, CR-010, CR-011, CR-014, CR-015, CR-016, CR-017, CR-019, CR-020, CR-021, CR-022, CR-023, CR-026, CR-030). Each entry preserves the original problem statement and direction; Status line names the closing commit.
...
**Active CRs:** see `CHANGE_REQUESTS.md` (19 entries — CR-003 iso-energy, CR-004 graphing, CR-005 fan control, CR-007 carbon variance, CR-008 REM↔OWL, CR-009 web client, CR-012 calibration history, CR-013 prev-run drilldown, CR-018 T2/T3 historical, CR-024 probe button, CR-025 RT Linux, CR-027 tier copy, CR-028 confidence model, CR-029 encoding rigor, CR-031 portability, CR-032 per-mode CO₂e rows, CR-033 curated demo video, CR-034 unified results card, CR-035 encode progress bar). Closed CRs in `CHANGE_REQUESTS_CLOSED.md`. CR-019 resume-job hook deferred to a follow-up CR — capture if/when it surfaces in real visitor traffic.
...
- **S23 (2026-05-07):** **CR-022 fully resolved + CR-026 anon-integrity + parameters audit for Tania.** Three commits on `main`. **Part 1 (`9e1d076`)** — CR-022 closed by ffmpeg-master upgrade. New `ffmpeg_bin` setting (default `/usr/local/bin/ffmpeg-master`) routed via `_ffmpeg_bin()` across all 6 PRESETS + `apply_custom_cmd` (rewrites a leading bare `ffmpeg` token so calibration templates pick up the upgraded binary). `_maybe_cap_vaapi()` and `gpu_encode_max_s` deleted; result dict reports `ffmpeg_version` in place of `gpu_capped_at_s`. Methodology page drops the surface-pool callout and the P110-resolution caveat no longer references the cap. First post-resolution n=6 calibration: idle 2.41%→2.26%, cpu 1.33%→0.66%, **gpu 4.77%→0.95%** — confirms S22's GPU CV was inflated by the 30s cap producing only ~30 polls per run. **Part 2 (`6d14f1e`)** — `docs/wattlab_parameters_audit.md`: every settings.json + in-code parameter classified Arbitrary / Empirical / Calibrated / Constrained, with a "path to principled" column. Responds to Tania's meeting question on `baseline_polls=8` ("how should I think about these versions?"). Lives alongside her `wattlab_traffic_light_confidence.md` and `wattlab_service_overview.md`. **Part 3 (`caa025b`)** — CR-026 anonymous-tier integrity pass. Five coordinated changes: (A) `save_result()` persists `visitor_key`; `list_results()` / `load_result()` scope by it (None=Lab=unfiltered; pre-CR-026 records invisible to non-Lab); `/results/{type}/list`, `/results/{type}/{id}/download.{json,csv}`, and `/image` page server-side prev-runs all flow through. (B) `CUSTOM_UPLOAD` moved Tier.Anonymous → Tier.Member; `/video` upload form gets the existing Members-only lock affordance. (C) New `WORKING_NAV` cap retires the raw tier compare on the home redirect; `/video/upload` size cap simplifies to member-only since Anonymous can't reach the route. (D) New test walks every FastAPI route and asserts `Depends(requires(...))` or explicit waiver — catches "shipped a new endpoint without a gate." `_visitor_key` → `visitor_key` (public) in `queue_control`. 196 tests passing. Phase E (curated content expansion) deferred. Then re-pruned `CHANGE_REQUESTS.md` (1036→918 lines): retired CR-020 to closed (superseded by CR-028 Phase 2), moved CR-026 to closed, updated CR-022 closed entry to reflect full resolution, refreshed CLAUDE.md cross-refs and deferred items.
