"""
make_football_iso_vmaf_table.py — the same iso-VMAF interpolation as
make_iso_vmaf_table.py, standalone for the 2026-09-03 football sweep instead of the
...
Usage: python3 docs/smpte_2026/make_football_iso_vmaf_table.py
Writes football_iso_vmaf_table.csv next to this script.
"""
import csv
import json
import sys
...
ARTIFACT = Path(__file__).resolve().parents[2] / "results" / "calibration" / "_staging" / \
    "encode_parity_football_2026-09-03_final.json"
VMAF_TARGETS = [88, 90, 92, 94, 96]
PROFILES = ["cpu", "gpu_baseline", "gpu_tuned"]
COMPLEXITY = "sport-broadcast (SI~48.3/TI~10.3, measured 2026-09-04)"
...
def main():
    ...
    out_path = Path(__file__).parent / "football_iso_vmaf_table.csv"
        ...
        for codec in ("h264", "h265", "av1"):
            for profile in PROFILES:
                ...
                for t in VMAF_TARGETS:
                    ...
                    w.writerow(["football_30s", COMPLEXITY, codec, profile, t,
