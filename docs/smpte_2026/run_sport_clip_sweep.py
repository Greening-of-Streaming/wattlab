#!/usr/bin/env python3
"""
run_sport_clip_sweep.py — SMPTE-paper-only encode-parity campaign for the
4K/16:9-matched sport clip replacing Kranjska (Tania, 2026-08-28).
...
# (yet) wired in here.
CLIP_KEY = "readysetgo_30s"
SOURCE_CLIP = Path("/home/gos/wattlab/test_content/readysetgo_30s_looped.mp4")
# ---------------------------------------------------------------------------

ARTIFACT_DIR = REPO_ROOT / "results" / "calibration"
OUT_ARTIFACT = ARTIFACT_DIR / f"encode_parity_readysetgo_{datetime.now(timezone.utc).date()}.json"
...
def print_recipes() -> None:
    camp = campaign()
    n = camp.count()
    log(f"=== sport-clip full sweep: {n} rows ===")
...
async def run_real(use_lab_session: bool) -> int:
    ...
    PAUSE_FLAG.write_text("docs/smpte_2026/run_sport_clip_sweep.py — SMPTE sport-clip campaign\n")
    if use_lab_session:
        log(f"Setting {LAB_SESSION_FLAG} (non-Lab job submission refused, browsing stays open)")
        LAB_SESSION_FLAG.write_text("sport-clip parity campaign\n")
