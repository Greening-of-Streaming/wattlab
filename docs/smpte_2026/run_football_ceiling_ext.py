...
OUT_ARTIFACT = ARTIFACT_DIR / "_staging" / f"encode_parity_football_ceiling_ext_{datetime.now(timezone.utc).date()}.json"
...
# CEILING EXTENSION (2026-09-04 00:55, additive, separate artifact — Tania's rule: decide after scoring the
# real VMAF): on the frozen ladder football tops out at VMAF v0 91.6 (AV1, 7.5 Mbps) and 91.4 (HEVC GPU,
# 10 Mbps), so VMAF 92 is unreachable for 5 of 9 cells. Same pattern as bin/run-bitrate-ceiling-ext.py.
MATCHED_BITRATES = {"av1": [9000, 11000, 13000], "h265": [12000, 14000, 16000]}
