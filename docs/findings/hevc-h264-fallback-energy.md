...
headline: "Dropping HEVC for H.264 cuts encode energy ~47–68% but needs up to ~2× the bitrate for the same quality — the encoder's energy saving reappears as a storage and bandwidth requirement"
...
confidence: green
scope: "Device layer only (GoS1: AMD Ryzen 9 7900 + RTX 5080, NVENC). Network, CDN, and CPE excluded. No amortised training cost. The delivery-side figure is reported as a DATA requirement (bitrate / bytes per stream), NOT as network energy — GoS does not treat extra bandwidth as necessarily more network energy (the relationship is non-trivial)."
...
caveats:
  - "Encode energy is measured directly at the wall (Tapo P110, 1 Hz, OWL focus-mode protocol, all rows 🟢). The delivery-side figure is a DATA requirement (bitrate / bytes per stream), not an energy claim. OWL is device-layer only and does not assert that more bandwidth means more network energy — that relationship is non-trivial and contested, so we leave it unquantified rather than apply a per-GB intensity factor."
...
- **In delivery, the downgrade is more demanding on data — and that scales with the audience.** H.264 needs more bits for the same perceptual quality. The encode saving is paid once per title; the extra bytes are stored once but moved on every stream, every viewer-hour. We stop at the data requirement (see scope on why we don't convert bandwidth into network energy).

This is the first OWL finding that pairs measured encode energy with the *bit-efficiency* axis to reason about a real industry decision, and it makes the trade-off legible: a codec choice is not a single "more/less energy" number — it trades a one-time, measurable compute saving for a recurring data requirement on every stream.
...
Concretely, the BBB-class +43% is ~1.5 Mbps extra, roughly **+1.3 GB on a two-hour stream** — stored once and moved on every play.
...
- **Network / delivery energy.** OWL is device-layer only and stops at the data requirement. We do not convert the bitrate gap into network/CDN energy: GoS does not assume that more bandwidth means more network energy — that relationship is non-trivial and contested. The bitrate delta is reported as data (bytes), not Wh.
