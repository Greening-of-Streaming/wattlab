...
for ax,(dev,label) in zip(axes,DEV):
    ...
    ev=[v[0] for v in eth if v[0] is not None]; wv=[v[0] for v in wifi if v[0] is not None]
    avg = (f"   avg Ethernet {st.mean(ev):.2f} W → Wi-Fi {st.mean(wv):.2f} W (+{100*(st.mean(wv)-st.mean(ev))/st.mean(ev):.0f} %)" if ev and wv else (f"   avg Wi-Fi {st.mean(wv):.2f} W" if wv else ""))
    ax.set_ylabel("ΔW above idle (W)"); ax.set_title(label+avg,fontsize=10,loc="left",fontweight="bold")
    ax.set_ylim(0,top)
