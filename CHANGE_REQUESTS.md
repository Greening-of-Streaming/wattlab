...
1. **Dependency currency / supply chain** — `requirements.txt` is a 2026-04 snapshot; `pillow==10.2.0` and `requests==2.31.0` predate named CVEs; no `pip-audit`/dependabot. *Verify:* run `pip-audit` against the live venv. Confirmed fixes land under **CR-066**. **PARTIAL VERIFY 2026-07-06:** `pip-audit` can't run in the dev sandbox (PEP 668 externally-managed system Python; the live venv is on GoS1). Manual check of the two named pins stands: **`pillow==10.2.0`** is < 10.3.0 (CVE-2024-28219, `_imagingcms.c` buffer overflow) → bump to ≥10.3.0; **`requests==2.31.0`** is < 2.32.0/2.32.4 (CVE-2024-35195 `verify=False` persistence, CVE-2024-47081 `.netrc` credential leak) → bump to ≥2.32.4. `python-multipart==0.0.22` is *past* the 0.0.18 DoS fix (OK). The transformers/torch/diffusers/onnxruntime model stack cannot be asserted from version strings alone — **run `pip-audit` on the GoS1 venv** for the authoritative list before bumping (a `requirements-dev.txt` with `pip-audit` is the CR-068 hook). ⚠ torch is a CUDA wheel (`+cu128`) — bump pillow/requests independently, don't blanket-upgrade.
...
5. **Concurrency/races in the job machinery** — no `Lock` anywhere in `queue_control`; per-visitor cap is check-then-act; `focus_mode_exit` future discarded; world-writable `/tmp/gos-measure.lock` is squattable. *Verify* (the single-worker uvicorn model may bound some of this), then fixes land under **CR-067** item 7. **VERIFIED 2026-07-06 — largely refuted:** `enqueue` is a synchronous `def` with **no `await` between the cap-check (`queue_control.py:136`) and the `pending_queue.append` (`:155`)**, so on the single-worker uvicorn event loop no coroutine can interleave across the check-then-act — it is atomic *by construction*. **Do NOT add a `Lock` to `enqueue`** (it would be cargo-cult); if the deployment ever goes multi-worker/async-enqueue, revisit. The genuine residue is minor and already routed to **CR-067**: (a) `focus_mode_exit` fire-and-forget with the future discarded (an exception there silently leaves systemd units stopped — add a done-callback/log); (b) world-writable `/tmp/gos-measure.lock` is squattable (local-only, `low`). Cross-thread `jobs` writes from executor halves are single-key GIL-atomic under the disciplined-access invariant (§3.1) — not corrupting. Net: one `low` reliability nit (a), one `low` local-security nit (b); the headline race does not exist.
